...
export default defineComponent({
  ...
  setup() {
    ...
    //http://pinyin-pro.cn/
    watch(() => dailyTrainCarriage.value.name ,()=>{
      if(Tool.isNotEmpty(dailyTrainCarriage.value.name)){
        dailyTrainCarriage.value.namePinyin = pinyin(dailyTrainCarriage.value.name, { toneType: 'none' }).replaceAll(" ", "");
      }else {
        dailyTrainCarriage.value.namePinyin = "";
      }
    }, {immediate: true})

    //自动计算停车时长 dayjs
    watch(() => dailyTrainCarriage.value.inTime ,()=>{
      let diff = dayjs(dailyTrainCarriage.value.outTime, "HH:mm:ss").diff(dayjs(dailyTrainCarriage.value.inTime,"HH:mm:ss"), "seconds");
      dailyTrainCarriage.value.stopTime = dayjs('00:00:00', "HH:mm:ss").second(diff).format("HH:mm:ss");
    }, {immediate: true})
    watch(() => dailyTrainCarriage.value.outTime ,()=>{
      let diff = dayjs(dailyTrainCarriage.value.outTime, "HH:mm:ss").diff(dayjs(dailyTrainCarriage.value.inTime,"HH:mm:ss"), "seconds");
      dailyTrainCarriage.value.stopTime = dayjs('00:00:00', "HH:mm:ss").second(diff).format("HH:mm:ss");
    }, {immediate: true})
